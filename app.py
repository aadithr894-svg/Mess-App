...
def generate_bills():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            bill_month        = request.form.get('bill_month')  # e.g. '2025-09'
                ...
                cur.execute("""
                    INSERT INTO bills
                      (user_id, bill_date, daily_amount,
                       ...
                       reduction_amount, establishment_fee,
        ...
        if request.method == 'POST' and 'user_id' in request.form:
            ...
            for uid, fine_amount in zip(user_ids, fines):
                ...
                if fine_val > 0:
                    ...
                    """, (uid, today, meal_type, fine_val))
                    inserted += 1
            conn.commit()
            flash(f"✅ Fines recorded for {meal_type.capitalize()} ({inserted} user(s)).", "success")
            return redirect(url_for('add_fine', meal_type=meal_type))

    except Exception as e:
        if conn:
            conn.rollback()
        current_app.logger.exception("add_fine: DB error")
        flash(f"Database error: {e}", "danger")
    finally:
        if cur:
            cur.close()
...
def user_mess_skips():
    """
    Show ALL mess skips (every user) with optional month filter,
    ...
    # Base query
    query = """
        SELECT user_id, skip_date, breakfast, lunch, dinner
        FROM mess_skips
    """
    params = []

    # Optional month filter like 2025-10
    if selected_month:
        query += " WHERE DATE_FORMAT(skip_date,'%%Y-%%m') = %s"
    ...
    for r in rows:
        r["skip_date_display"] = r["skip_date"].strftime("%Y-%m-%d")
        ym = r["skip_date"].strftime("%Y-%m")
        skips_by_month[ym].append(r)

    # Build dropdown of distinct months
    conn = mysql_pool.get_connection()
    cur = conn.cursor()
    cur.execute("""
